...
{
#if defined(BOOST_USE_SEGMENTED_STACKS)
    __splitstack_getcontext( palloc_.segments_ctx);
#endif
}

coroutine_context::coroutine_context( ctx_fn fn, preallocated const& stack_ctx) :
    palloc_( stack_ctx),
...
#if defined(BOOST_USE_SEGMENTED_STACKS)
    __splitstack_getcontext( palloc_.segments_ctx);
    __splitstack_setcontext( other.palloc_.segments_ctx);

    intptr_t ret = context::jump_fcontext( & ctx_, other.ctx_, param, preserve_fpu);

    __splitstack_setcontext( palloc_.segments_ctx);
