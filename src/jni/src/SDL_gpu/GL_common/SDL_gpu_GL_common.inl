...
{
	Uint16 tex_w = 0, tex_h = 0;
	float w = 0;
	float h = 0;
	float x1 = 0, y1 = 0, x2 = 0, y2 = 0;
	float dx1 = 0, dy1 = 0, dx2 = 0, dy2 = 0;
	GPU_CONTEXT_DATA* cdata = 0;
	float* blit_buffer = 0;
	unsigned short* index_buffer = 0;
	unsigned short blit_buffer_starting_index = 0;
	int vert_index = 0;
	int tex_index = 0;
	int color_index = 0;
...
static Uint32 LinkShadersEx(GPU_Renderer* renderer, Uint32 *shader_objects, int count)
{
    #ifndef SDL_GPU_DISABLE_SHADERS
	int i;
    GLuint p = glCreateProgram();
	...
	for (i = 0; i < count; i++)
		glAttachShader(p, shader_objects[i]);
...
#define SET_COMMON_FUNCTIONS(renderer) \
 ...
 \
    ...
    renderer->LinkShadersEx = &LinkShadersEx; \
