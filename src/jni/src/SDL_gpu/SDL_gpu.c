...
Uint32 GPU_LinkShadersEx(Uint32 *shader_objects, int count)
{
	if(current_renderer == NULL || current_renderer->current_context_target == NULL || current_renderer->LinkShadersEx == NULL)
		return 0;
	
	return current_renderer->LinkShadersEx(current_renderer, shader_objects, count);
}
